...
from day_20_grove_positioning_system.helper_functions import move_forward, move_backward, find_item
...
if __name__ == '__main__':
    ...
    for line in file:
        line = line.replace("\n", "")
        numbers.append([int(line) * 811589153, idx])
        idx += 1

    file.close()
    amount_items = len(numbers)
    print("Mix file 10 times...\nProcess may take 1 or 2 minutes...")
    # Step 2: Mix 10 times
    for z in range(10):
        for idx in range(amount_items):
            ...
            if value >= 0:
                # Simplification: Reduce amount of moves that produces the equivalent result
                iterations = value % (amount_items-1)
    ...
    for idx in pos_to_check:
        total += numbers[idx][0]
    print("Sum of the Three Numbers that form the Grove Coordinates (Solution Part Two): ", total)
